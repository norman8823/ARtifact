...
- **⚠️ Only iOS 18.x simulators work — every iOS 26 simulator is an invalid destination, permanently.** The Viro plugin (`withViroIos.js`) injects `EXCLUDED_ARCHS[sdk=iphonesimulator*] = arm64` on every prebuild, forcing **x86_64-only** simulator builds; iOS 26 simulator runtimes dropped x86_64, so they never appear in the destination list. Expo auto-selects a *booted* device, so leaving an iOS 26 sim booted produces `xcodebuild: error: Unable to find a destination matching the provided destination specifier` — which reads like a broken project and isn't one; the build never starts. Fix: `xcrun simctl shutdown <udid>` and pass `--device "iPhone 16 Pro"` (or any 18.x device). **This gets worse when Apple retires the 18.6 runtime** — at that point the arm64 exclusion has to be undone rather than worked around.
...
- **Lint:** `npm run lint`. **Tests:** `npm test` (jest-expo; pure-logic units in `src/utils/__tests__/` and `src/config/__tests__/`). **Typecheck:** `npx tsc --noEmit`. CI (`.github/workflows/ci.yml`) requires **all three** on PRs/pushes to `development` — the baseline is clean (0 type errors, 0 lint errors, 86 tests), so keep it that way rather than reintroducing `continue-on-error`.
...
- **Signing team:** `app.json` pins `ios.appleTeamId` = **`S3UAQ48824`** (Artifact Technologies LLC). **The pin alone signs nothing** — `appleTeamId` is consumed at *prebuild*, which writes `DEVELOPMENT_TEAM` into `ios/ARtifact.xcodeproj`; `expo run:ios` then reuses whatever the project already has. So after changing it, **run `npx expo prebuild -p ios --clean` or the build keeps signing under the old team while the config claims otherwise** (observed 2026-08-05). `ios/` is gitignored, so a fresh clone needs the prebuild too. Verify with `grep DEVELOPMENT_TEAM ios/ARtifact.xcodeproj/project.pbxproj`, and confirm the build log's `Auto signing app using team(s):` line — never `app.json` alone. **Why it is pinned at all:** with the key absent, `expo run:ios` silently writes whatever team it auto-selects into `app.json` — it wrote the owner's *personal* team `KXDG349VZC` on 2026-08-05. An explicit pin makes that substitution impossible. Local *device* builds additionally need an Apple ID with LLC membership signed into Xcode (no LLC cert exists on the owner's machine); simulator builds don't sign and work regardless.
...
- **Scan path:** `app/scan.tsx` → multipart POST to **Railway Flask CNN** `https://api.artifactar.com/predict` (host in `src/config/scanApi.ts`, overridable via `EXPO_PUBLIC_SCAN_API_URL`) → response adapted to the legacy Rekognition shape (`labels[{Name: artworkId, Confidence}]`) → `useScanSuccess` awards 100 XP on first visit + updates quest progress. Railway scales to zero → the only real server cold start in the app, scan-only. The Rekognition Lambda in `amplify/backend/function/analyzeImage` is **dead code**, kept intentionally.
...
**🔌 Scan backend — fully LLC-owned as of 2026-08-05. The partner dependency is gone.** The Flask CNN service that makes scanning work used to run in the former partner's Railway project, from his private GitHub repo, under a Railway-generated `artifact-server-production.up.railway.app` hostname compiled into the app. All three legs were replaced in one day (details in `~/main/ARtifact-server-notes.md`, deliberately outside the repo):
- **Source:** `norman8823/ARtifact-server`, private, full history (HEAD `bf7e721`, incl. the 48.7MB model). His repo remains as `upstream`; his access is now irrelevant in both directions.
- **Compute:** our own Railway project, building from our repo. Green build needed one service variable, `MISE_PYTHON_GITHUB_ATTESTATIONS=false` — the `Pipfile`'s Python 3.9.18 pin predates GitHub artifact attestations, so Railway's builder hard-fails verifying them.
- **Address:** `https://api.artifactar.com`, Cloudflare CNAME (**unproxied — the proxy intercepts Railway's cert validation, leave it grey**) plus a `_railway-verify` TXT record that **must not be deleted**, or renewal breaks ~60–90 days later.
- **The app:** `src/config/scanApi.ts` now defaults to `https://api.artifactar.com`. **This had to ship before App Store launch** — after launch, the same repoint is a forced-update campaign rather than a one-line diff.
- **Server code is unchanged.** Contract is still `POST /predict`, multipart, field `image` → `{prediction, confidence, success}`; health check is `GET /` (**there is no `/health`**, whatever the server README says); below the 0.40 threshold `prediction` is `"Unknown"` with `success` still true — a pass, not an error.
- **`GET /` is liveness, not readiness** — it returns "Running." unconditionally with no reference to the model, so a green deploy plus a 200 is compatible with every scan being broken. Verify a deploy with a real `/predict` call; the load-failure string is `"Model or class names are not available."` (HTTP 500).
- **No EAS override exists — checked 2026-08-05.** `eas.json` sets `"environment": "production"`, so an `EXPO_PUBLIC_SCAN_API_URL` in the Expo project would silently beat the compiled-in default; there isn't one (nine vars, none of them the scan host). The default is therefore load-bearing in production. **Don't add the var** unless a build needs a different host.
- **No OTA path.** `expo-updates` is not installed and no `updates` config exists, so the host can only change by shipping a binary. That is why the repoint had to land pre-launch.
...
- **Test harness + strict CI** — jest-expo, pure logic in `src/utils/` (`questProgress`, `rankUtils`, `scanAdapter`, `xpAward`, `premiumAccess`, `featuredQuest`, `accountDeletion`), plus `src/config/` (`scanApi`), GitHub Actions on PRs to `development`. **All three jobs required**: 86 tests, `tsc --noEmit` (0 errors), lint (0 errors).
...
- **Scan backend repointed** at `https://api.artifactar.com` — see the Scan backend block above. Regression-guarded by `src/config/__tests__/scanApi.test.ts`.
...
- *Blocked on the user:* (1) **App Store Connect** — create the non-consumable `com.artquest.ARtifact.premium.lifetime`; until it exists `fetchProducts` returns an empty array; (2) add `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_REGION` to `.env` so `markPremiumQuests.js` can run (dry-run first); (3) **TestFlight/device QA** of purchase, restore and refund — IAP does not work in the Simulator, and the paywall needs a signed-in account.
...
- **⚠️ Managed login needs a branding style or every hosted-UI page 403s.** Without one, `/login` returns *"Login pages unavailable. Please contact an administrator."* and the app shows a **blank, stuck web sheet** with no JS error. This cost real debugging time: passing `identity_provider=SignInWithApple` skips that page and goes straight to Apple, so a curl test of `/oauth2/authorize` passes while the app fails.
- **⚠️ `phone_number` is a required pool attribute and Apple supplies none.** Cognito forces a mapping for every required attribute, so it is mapped to Apple's `sub`. **This is the most likely failure point** — if Cognito validates that field as E.164 it will reject federated user creation at first sign-in. The fix would be a single PreSignUp trigger writing the same dummy `+10000000000` that email signup already sends (`useAuth.ts:99`) — one function, not the four-trigger CUSTOM_AUTH design. **Root cause is a pool-creation mistake:** `phone_number` was never required by Cognito; whoever set the pool up believed it was, and `Required` is immutable.
- **Apple's name arrives only on the first authorization** and the app client can only write `email`/`phone_number`, so Cognito never persists it. `useAppleSignIn` falls back rather than assuming it is present.
- Still owed: 1A.6 Apple token revocation on account deletion — required by Apple once SIWA ships, and it needs a server-side piece the client-only deletion flow does not have.
...
- **The scan host default must stay a hostname we own.** `FALLBACK_SCAN_API_URL` in `src/config/scanApi.ts` is what actually ships (`EXPO_PUBLIC_SCAN_API_URL` is unset in `.env`), so it is compiled into every binary. Pointed at a domain we control, losing or moving the recognition service is a DNS change every installed build follows on its next scan; pointed at a vendor-generated hostname, the same event needs an App Store release and strands users until they update. That is the whole reason the 2026-08-05 repoint was urgent — don't undo it by hardcoding a provider subdomain again.
