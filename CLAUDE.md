...
- ⚠️ **Owner action before the release build:** confirm `EXPO_PUBLIC_SCAN_API_URL` is not pinned to the old host in the EAS **production** environment (`eas.json` sets `"environment": "production"`). It would silently override the new default — and it cannot be checked from this repo.
