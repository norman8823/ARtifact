...
### 21. No privacy policy, and the App Privacy label has never been filled *(backlog 0.6)*
Discovered 2026-07-27 while assessing submission readiness — it was absent from all three docs. App Store Connect requires a privacy-policy URL and a complete App Privacy declaration. The app collects account email, camera images (uploaded to a third-party Railway endpoint), and — via `sendDefaultPii: true` — IP and identifiers to Sentry. None of that is documented or disclosed anywhere.

### 22. Declared-but-unused permissions, and Sentry records screens *(backlog 0.7, 0.8)*
`app.json` declares two location usage strings and ships `expo-location`, but **nothing imports it**; `expo-web-browser` appears unused too. Apple rejects unused permission requests, and each one adds an App Privacy obligation. Separately, `mobileReplayIntegration()` is active at a 10% session / 100% error sample rate, so real user screens are being recorded — fine for a beta, but undisclosed and worth an explicit decision before taking payments. `PrivacyInfo.xcprivacy` exists only under the gitignored `/ios`, so it is regenerated and effectively uncontrolled.
