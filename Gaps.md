...
**✅ CONFIRMED FIXED ON iOS 27 (2026-08-07).** The reporter verified the custom control pops where the system button did not. The approach is right; it was briefly reverted on 2026-08-06 for the cosmetic problem below and is now restored with the fix.

**Cosmetic defect shipped in 1.0.53, fixed 2026-08-07.** iOS 26+ wraps navigation-bar items in a Liquid Glass circle **and there is no opt-out** — no glass/liquid prop exists in `react-native-screens` 4.16 or `@react-navigation/native-stack`. The container sizes itself to whatever view it is handed, so `HeaderBackButton`'s asymmetric padding (`paddingRight: 8`, no fixed size) produced a visibly off-centre arrow. It is now a **square 30×30 centred box**, with a 2pt nudge for `chevron-left`'s left-weighted glyph. **Never give this button asymmetric padding.**

**Historical note on the reverted attempt:** On the owner's iOS 26.5.2 device the custom button rendered inside a **heavy dark Liquid Glass circle** that read as broken. iOS 26+ wraps navigation-bar items in that container and **there is no opt-out** — neither `react-native-screens` 4.16 nor `@react-navigation/native-stack` exposes any glass/liquid prop. Reported on the same build: **artDetail taking ~3x longer to open, several seconds** — hosting a React view inside the native bar on every push is the prime suspect, though unconfirmed (artDetail is also uncached legacy code, Gaps #9, so it was never fast). Both symptoms appeared together with the change and the change is reverted; **whether the slowdown actually came from it still needs confirming on a build without it.**

**➡️ The replacement is the option that was scoped as the fallback: `headerShown: false` on pushed screens with a header rendered entirely in React.** That owns its own styling — no system container to fight — and removes `react-native-screens`' navigation bar from the path entirely, which is the actual goal. `components/HeaderBackButton.tsx` is kept for it (currently unused). Note `fullScreenGestureEnabled` is also implemented natively and may share the iOS 27 exposure.

**⬜ Residual risk (of the reverted approach, kept for the record).** `headerLeft` still rendered inside the native navigation bar, so it was never fully insulated from a UIKit regression. If iOS 27 confirmation fails, the next step is `headerShown: false` on pushed screens with a header rendered entirely in React — no dependence on `react-native-screens`' bar handling at all. Note `fullScreenGestureEnabled` is also implemented natively and may share the same exposure.
...
### 24. Stripping SDK-referenced purpose strings breaks App Store delivery — ✅ **FIXED AND CONFIRMED 2026-08-06**
**Build 1.0.53 delivered successfully with no ITMS-90683.** Only the known-benign `ITMS-90076` keychain warning remained, which Apple lists as non-blocking. That upload is the confirmation — the fix could not be verified any other way.
