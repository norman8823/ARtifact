...
**Cheapest first step: ask him to add the LLC as a collaborator on the repo, then clone it immediately.** One click for him, and a local clone survives him deleting or archiving it — that is most of the practical protection for near-zero friction. It does *not* substitute for the steps below: repo access is not deploy access, and if his Railway account lapses the service dies with the source safely in hand and no way to restore the hostname. A GitHub repo *transfer* is also about one click, so ask for ownership rather than access if he has no reason to keep it.
