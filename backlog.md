...
It also undercuts a compliance claim: 0.6's "images are not retained" is about to be published in a privacy policy, but it depends on code the LLC can neither see nor freeze. A change on his side would silently make our published policy false.

Settle: who owns the repo, who owns the Railway project and its billing, and whether the LLC gets the source or at minimum a transfer of the deployment. Do it in the same conversation as the 0.6 answer — this is the last moment there's a reason for him to be responsive.

**✅ Source secured 2026-08-04 — cloned to `/Users/norman/main/ARtifact-server` at HEAD `bf7e721`.** Owner was already a collaborator, so this needed nothing from the partner. The clone survives him deleting or archiving the repo. **Still to do: push a copy to a remote the LLC owns** — a local-only clone dies with the laptop. Note repo access is *not* deploy access: if his Railway account lapses, the service dies with the source safely in hand and no way to restore the hostname. A GitHub repo *transfer* is about one click, so it's still worth asking for ownership if he has no reason to keep it — but it is now a nice-to-have, not protection.

**Cost split, confirmed 2026-08-04 and considered fair — not the thing to change.** The partner pays a nominal Railway fee; the LLC pays ~$10/month for AWS. **Cost is not the deciding variable here** — at this scale, taking Railway over would be a rounding error next to the AWS bill, so the question is control and continuity, never money. The realistic failure mode is not unfairness but disengagement: a small recurring charge lapsing quietly a year from now, while paying users can't scan. The custom domain is what makes this split *durable* — it turns "who runs the compute" into a decision made calmly later rather than one forced during an outage.

**Owner does not want to operate the Railway service** (already carrying the AWS stack). That's a legitimate call and it does **not** have to be resolved now — but note the responsibility already sits with the LLC regardless of whose account runs the box: we own the app, we're about to charge for it, and when scan breaks the complaints, reviews and refunds are ours. What's missing is not responsibility but the *ability to act* on it.

**The cheap resolution is DNS, not ownership — do this one regardless:**
- **Put a custom domain we control in front of his deployment.** `api.artifactar.com` (same domain as the privacy policy) → his Railway service; the app ships pointing at our domain. He keeps operating it; if it ever dies or he disappears, we repoint DNS at a replacement and **every shipped build follows** — no app update, no emergency release, no stranded users. This turns "who runs the compute" into a reversible decision made later, calmly, instead of under pressure.
- ~~Replace the literal at `scan.tsx:186` with an `EXPO_PUBLIC_*` var~~ — **✅ done 2026-08-04.** `src/config/scanApi.ts` exports `SCAN_PREDICT_URL`, read from **`EXPO_PUBLIC_SCAN_API_URL`** and falling back to the current Railway host. Deliberately **not** added to the required-env-var list in `src/aws/config.ts`, which *throws* on a missing var — a value absent from the EAS build environment would brick launch, so a missing value degrades to today's behaviour instead. **The code half is shipped; the DNS half still needs the partner.** When `api.artifactar.com` exists, set the env var — no code change.
...
**Action: push the clone to a remote the LLC owns.** Removes the delete/revoke risk properly. Tell him rather than doing it quietly. Keep it a **separate repo, not merged into this one** — CI here requires jest + `tsc --noEmit` + lint to all pass, and adding a Python service means reworking those jobs or adding path filters for no present benefit; Railway also deploys from a repo root.

**The ask to the partner has collapsed to one thing: the custom domain.** Source is secured, retention is verified first-hand, and the log-retention question was dropped (0.6). All that genuinely requires him is adding `api.artifactar.com` as a custom domain on the Railway service so shipped builds stop hardcoding a hostname in his account. *Possible snag: Railway custom domains may require a paid plan.*
