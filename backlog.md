...
3. **"Images … are processed and stored securely" over-claims — owner confirmed no storage is wanted or needed.** Verified 2026-07-27: the app stores **no** images. The only S3 `uploadData` call sits inside the commented-out dead Rekognition block (`scan.tsx:236`); the live path (`analyzeWithFlask`) POSTs the photo to Railway `/predict` and retains nothing beyond expo-camera's transient cache file. Owner intent is no storage at all, so the policy should say images are transmitted for matching and not retained — **pending confirmation that the Railway Flask service doesn't persist them.** That service lives in a separate repo the partner controls, so ask him **before he disengages post-transfer**; this is the last dependency on him.

Apple's test is retention *beyond the time necessary to service the request*, so the question has to be asked at that resolution — "do you store images?" is too loose. Specifically: (1) does `/predict` write the upload to disk, and is it deleted before the response returns or left in a temp dir; (2) is the image or any derivative (thumbnail, tensor, embedding) written to S3, a database or a Railway volume; (3) does anything **log** the image or raw request body, including an error tracker capturing payloads on exception; (4) are uploads kept for retraining, evaluation or debugging; (5) is the image forwarded to any third party; (6) separately, are IPs or device identifiers logged, and for how long. If anything is retained: retention period and deletion mechanism. Fast check in that repo if he'd rather look than recall: grep the predict path for `open(`, `.save(`, `boto3`, `s3`, `upload`, `tempfile`, `NamedTemporaryFile`, plus whatever logging wraps the route — framework-level temp files and exception handlers dumping payloads are the usual culprits.

**Outcomes:** nothing retained → images are not "collected" under Apple's definition, and the policy says transmitted-for-matching-not-retained. Anything retained → must be disclosed in App Privacy *and* the policy. IPs logged → a separate disclosure, alongside the Sentry one already owed.
