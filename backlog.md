...
**Order matters — do it this way:**
1. **Transfer the Railway project itself; do not recreate it from source.** The endpoint is a bare string literal at `scan.tsx:186` (no env var, no remote config), and that hostname is a Railway-generated subdomain tied to his project. Redeploying under a different account yields a *different* subdomain, and every already-shipped build keeps calling the old one — scanning dies for everyone who hasn't updated, unfixable server-side. A project transfer carries the domain with it and nobody notices.
2. **Then put a custom domain in front of it** — e.g. `api.artifactar.com`, on the domain already used for the privacy policy — and replace the literal with an `EXPO_PUBLIC_*` var pointing at it. Ship with the premium release. After that the backend can move host, account or provider freely. Until then, one hardcoded hostname is the difference between a working app and a dead scan feature.
3. **Then take the source**, into a repo under the LLC org. **Separate repo, not this one** — CI here requires jest + `tsc --noEmit` + lint to all pass, and adding a Python service means reworking those jobs or adding path filters for no present benefit; Railway also deploys from a repo root. Check the CNN weights' size and whether they're committed before cloning — large `.h5`/`.keras`/`.pt` blobs in git history are what turn this into a Git LFS exercise.
