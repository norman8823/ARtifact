...
- **Replace the literal at `scan.tsx:186` with an `EXPO_PUBLIC_*` var** pointing at that domain, and ship it with the premium release. Today the endpoint is a bare string (no env var, no remote config) whose hostname is a Railway-generated subdomain tied to *his* project — so recreating the service under any other account yields a different subdomain and every already-shipped build keeps calling the old one. Unfixable server-side.
